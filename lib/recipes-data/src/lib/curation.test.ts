...
import {importCurationData} from "./curation";
...
describe("importNewData", ()=>{
  ...
  it("should upload the data to the right place and flush the CDN cache", async ()=>{
    await importCurationData("test-content", "some-region", "some-variant", null);
  ...
  it("should respect the date parameter if given", async ()=>{
    ...
    await importCurationData("test-content", "some-region", "some-variant", d);
